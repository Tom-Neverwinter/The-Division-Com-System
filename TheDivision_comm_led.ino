...
//did not use digispark neopixel library, uses adafruit library
...
Adafruit_NeoPixel strip_b = Adafruit_NeoPixel (16, PIN_4, NEO_GRB + NEO_KHZ800);
...
void setup() {
  ...
  strip_b.show(); // This initializes the NeoPixel library.
}
...
void loop() {
  ...
  for (int i = 0; i < NUMPIXELS; i++) {
    ...
    strip_a.setPixelColor(i, strip_a.Color(255, 50, 10));
    strip_b.setPixelColor(i, strip_b.Color(10, 50, 255)); // Resistance Blue
    strip_a.show(); // This sends the updated pixel color to the hardware.
    strip_b.show(); // This sends the updated pixel color to the hardware.
